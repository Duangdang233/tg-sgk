...
async function request(config, path, init = {}, signal) {
...
export default defineToolPlugin({
    ...
    tools: (tool) => [
        tool({
            ...
            description: "Verify that a Telegram target is a bot before any interaction. Human users, groups, and channels are rejected by the service.",
            parameters: Type.Object({ bot: Type.String({ minLength: 2 }) }),
            execute: ({ bot }, config, context) => request(config, "/v1/bots/inspect", { method: "POST", body: JSON.stringify({ bot }) }, context.signal),
        }),
        tool({
            name: "tg_send_message",
            label: "Send Message to Telegram Bot",
            description: "Send text or a command to a verified Telegram bot. This tool cannot message human accounts, groups, or channels.",
            parameters: Type.Object({
                bot: Type.String({ minLength: 2 }),
                text: Type.String({ minLength: 1, maxLength: 4096 }),
            }),
            execute: (params, config, context) => request(config, "/v1/messages/send", { method: "POST", body: JSON.stringify(params) }, context.signal),
        }),
        tool({
            ...
            execute: ({ bot, limit = 10 }, config, context) => request(config, `/v1/messages/recent?bot=${encodeURIComponent(bot)}&limit=${limit}`, {}, context.signal),
        }),
        tool({
            name: "tg_wait_update",
            ...
            execute: (params, config, context) => request(config, "/v1/messages/wait", { method: "POST", body: JSON.stringify(params) }, context.signal),
        }),
        tool({
            name: "tg_click_button",
            label: "Click Telegram Bot Button",
            description: "Click one button in a verified Telegram bot message, selected either by exact text or row and column.",
            parameters: Type.Object({
                bot: Type.String({ minLength: 2 }),
            ...
            execute: (params, config, context) => request(config, "/v1/buttons/click", { method: "POST", body: JSON.stringify(params) }, context.signal),
        }),
        tool({
            name: "tg_save_flow",
            label: "Save Telegram Bot Flow",
            description: "Save or update a confirmed fixed Telegram bot workflow. Use after the operator has approved the explored steps.",
            parameters: Type.Object({
            ...
            execute: (params, config, context) => request(config, "/v1/flows", { method: "POST", body: JSON.stringify(params) }, context.signal),
        }),
        tool({
            name: "tg_list_flows",
            ...
            parameters: Type.Object({}),
            execute: (_params, config, context) => request(config, "/v1/flows", {}, context.signal),
        }),
        tool({
            name: "tg_run_flow",
            label: "Run Telegram Bot Flow",
            description: "Run one previously confirmed fixed Telegram bot workflow. Fixed flows execute without model reasoning.",
            parameters: Type.Object({ flow_id: Type.String({ minLength: 2, maxLength: 64 }) }),
            execute: ({ flow_id }, config, context) => request(config, `/v1/flows/${encodeURIComponent(flow_id)}/run`, { method: "POST" }, context.signal),
        }),
        tool({
            name: "tg_get_history",
            ...
            execute: ({ limit = 50, target }, config, context) => {
